...
- block: { id, type, name, config, output_schema, connections }
  ...
  - config:
    ...
    - 插件块: { plugin_id: "xxx", prompt: "可选的输入模板" }
      ...
      - 如果上游数据格式与插件输入格式匹配，prompt 可以为 null
      - 如果不匹配，必须在 prompt 中使用模板变量转换，例如：
...
def _build_plugins_info() -> str:
    ...
    for plugin_info in enabled:
        ...
        lines.append("2. 配置输入模板（config.prompt）：")
        lines.append("   - 如果上游数据格式与插件输入格式不匹配，必须在 config.prompt 中使用模板变量转换")
        lines.append("   - 例如：如果插件需要 {\"symbol\": \"...\"} 但上游是 {\"stock_code\": \"...\"}，")
        lines.append(f"     则设置 config.prompt = '{{\"symbol\": \"{{{{input.stock_code}}}}\", \"market\": \"A\"}}'")
        lines.append("   - 如果上游数据格式已经匹配，可以留空 config.prompt")
        lines.append("3. 插件块的 output_schema 应该根据插件的输出格式设置 keys 列表，以便下游块引用")
